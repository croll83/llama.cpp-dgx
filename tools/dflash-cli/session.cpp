...
// ─── dflash_session: public library API ──────────────────────────
...
extern "C" dflash_session_params_t dflash_session_default_params(void) {
    dflash_session_params_t p{};
    p.max_ctx           = 4096;
    p.ddtree_budget     = 22;
    p.ddtree            = 1;
    p.fast_rollback     = 1;
    p.ddtree_chain_seed = 1;
    p.ddtree_temp       = 1.0f;
    p.kv_tbq            = 0;
    p.prefill_ubatch    = 0;
    return p;
...
extern "C" int dflash_session_reset(dflash_session_t * s) {
    if (!s) return -1;
...
extern "C" int dflash_session_kv_end(const dflash_session_t * s) {
    return s ? s->kv_end : 0;
}

extern "C" int32_t dflash_session_last_tok(const dflash_session_t * s) {
...
extern "C" int dflash_session_run(dflash_session_t * s,
    ...
    // RESET path: legacy test_dflash behavior (wipe cache / SSM / conv state).
    if (!append_mode && !s->first_iter) {
        if (dflash_session_reset(s) != 0) return -1;
    }
    s->first_iter = false;

    const int prefill_kv_offset = append_mode ? s->kv_end : 0;
    if (!append_mode) s->kv_end = 0;
    ...
    const int max_verify_tokens  = s->max_verify_tokens;
    const bool fast_rollback     = s->params.fast_rollback != 0;
    const bool ddtree_mode       = s->params.ddtree        != 0;
    const int  ddtree_budget     = s->params.ddtree_budget;
    const float ddtree_temp      = s->params.ddtree_temp;
    const bool ddtree_chain_seed = s->params.ddtree_chain_seed != 0;
    ...
    // daemon ready print (dropped in library path)

    // sg is session member, accessed via the  reference above

    // single-pass session_run (daemon loop removed)
        // prompt ingestion (dropped — caller provided  above)
